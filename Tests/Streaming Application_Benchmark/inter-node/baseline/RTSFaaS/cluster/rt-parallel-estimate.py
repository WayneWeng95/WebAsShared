...
print(f"=== launching 4 INDEPENDENT single-node {APP} runs CONCURRENTLY ===", flush=True)
...
for ip, name in NODES:
    ...
    print(f"  {name} ({ip}): {v}")
    if v: tot += v; ok += 1
print(f"\nPARALLEL ESTIMATE ({APP}, {ok}/4 nodes): sum = {tot:.1f} records/sec  (embarrassingly-parallel upper bound)")
