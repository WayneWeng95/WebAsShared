...
# Writes figs/cluster_throughput_overlay_8node.{png,pdf} (+ Tests/Figures/).
import os, shutil
import matplotlib
matplotlib.use('Agg')
...
DATA = {
    ...
    'rtsfaas':    {'mediareview': 40280,  'socialnetwork': 46303},   # 8-node load-balanced
    'was_async':  {'mediareview': 275608, 'socialnetwork': 231303},  # 8 nodes async @3.6M
    'was_sync':   {'mediareview': 230932, 'socialnetwork': 184833},  # 8 nodes sync @3.6M
}
SERIES = [
...
def main():
    ...
    for ext in ('png', 'pdf'):
        out = os.path.join(FIGS, f'cluster_throughput_overlay_8node.{ext}')
        fig.savefig(out, dpi=150, bbox_inches='tight', pad_inches=0.02)
        print('wrote', out)
        if ext == 'pdf':
            os.makedirs(TESTS_FIGS, exist_ok=True)
            shutil.copy(out, os.path.join(TESTS_FIGS, 'cluster_throughput_overlay_8node.pdf'))
