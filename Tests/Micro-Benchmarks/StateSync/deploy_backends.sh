...
write_env() {
    # Determine whether this is a loopback-only (local) Redis deployment.
    # bench.py reads REDIS_LOCAL_HOST/PORT for the local row and REDIS_HOST/PORT
    # for the remote row — they are distinct keys and must not collide.
    local is_local=0
    [[ "$BIND_IP" == "127.0.0.1" || "$BIND_IP" == "::1" ]] && is_local=1

    # Append to the env file so a local run (--bind 127.0.0.1) can coexist with
    # a previously copied remote backends.env without overwriting remote entries.
    {
        echo "# Generated by deploy_backends.sh on $(hostname) at $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
        echo "# bind IP for the backends started on THIS node:"
        echo "STATESYNC_BIND_IP=$BIND_IP"
        if [[ $WANT_REDIS -eq 1 ]]; then
            if [[ $is_local -eq 1 ]]; then
                echo "REDIS_LOCAL_HOST=$BIND_IP"
