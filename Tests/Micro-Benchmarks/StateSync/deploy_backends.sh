...
#
#   # Drive the remote node from the compute node in one shot (scp + ssh):
...
start_redis() {
    ...
    #   - persistence OFF (no RDB snapshots, no AOF) so writes never touch disk
...
main() {
    ...
    case "$cmd" in
        ...
        *) die "unknown command '$cmd' (try: up | down | status | remote | help)";;
    esac
}
