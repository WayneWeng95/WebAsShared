# finra_rules.py — the frozen 8-rule FINRA audit spec, in Python.
#
# A faithful port of Executor/guest/src/workloads/finra.rs so every Python-based
# baseline (Cloudburst, RMMap-ES) computes the SAME violations as our native
# system. Inputs/outputs are plain Python so they serialize cleanly through a KVS
# (the inter-stage state transfer the comparison measures).
#
#   parse_trades(csv_text) -> list[Trade dict]
#   REF_TABLE              -> the 20-symbol reference (avg price in cents)
#   audit_rule(trades, rule_id) -> violation count for that rule

# Reference avg price in cents — must match finra.rs `finra_fetch_public`.
REF_TABLE = {
...
def _cents(s):
    # mirror finra.rs parse_cents: integer cents, no float
...
def parse_trades(csv_text):
    """Parse the trades CSV (header + rows) into trade dicts, matching finra.rs."""
    trades = []
    for i, line in enumerate(csv_text.split('\n')):
        line = line.strip()
        if not line or i == 0:           # skip header / blanks
            continue
        p = line.split(',')
        if len(p) < 7:
            continue
