...
def splitter(meta):
    # Corpus parameterized so all systems count the same input (sanctioned by
    # the WordCount baseline README); falls back to the original dataset.
    text_path = os.environ.get(
        'WC_CORPUS',
        'datasets/OliverTwist_CharlesDickens/OliverTwist_CharlesDickens_French.txt')
    stage_name = 'splitter'
...
def mapper(meta):
    ...
    def mapper_es(meta):
        ...
        tick = cur_tick_ms()
        # Correctness fix: the upstream code wrote every mapper's partial to the
        # SAME key ('/tmp/wc.txt'), so with a shared KVS the partials clobber and
        # the reducer sums the survivor N times. Key it by mapper ID so the N
        # partials are distinct (DMERGE already keys by unique object id). Timing
        # (es/sd) is unaffected.
        out_file_path = '/tmp/wc_%d.txt' % ID
