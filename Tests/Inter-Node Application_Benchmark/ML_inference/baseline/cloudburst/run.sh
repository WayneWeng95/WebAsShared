#!/usr/bin/env bash
# run.sh — Cloudburst-on-Redis MNIST-inference sweep. One fresh process per (size,W) cell; rows
...
first=1; : > "$CSV"
echo "[cloudburst] sizes=[$SIZES] W=[$W_LIST]"
for NS in $SIZES; do
  ...
  for W in $W_LIST; do
    ...
    MLI_MODEL="$MODEL" MLI_DATA="$DATA" python3 "$HERE/infer.py" "$size_mb" "$NS" "$W" $hdr | tee -a "$CSV"
