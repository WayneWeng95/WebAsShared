#!/usr/bin/env python3
# plot.py — synchronous-SGD cross-system overlay: WebAsShared (ours) vs Cloudburst
...
#   ml_training_overlay.pdf : (1) throughput (M sample-grads/s) vs W at the largest
#                             size  (2) peak throughput per size bars
#   ml_training_bars.pdf    : the paper figure — one panel per size, four systems'
#                             best end-to-end training latency (s), bars in legend order.
#
...
def thr_vs_w(rows, size_mb):
    """(W, M sample-grads/s) for a size, sorted by W."""
    pts = []
    for r in rows:
...
def best_thr_by_size(rows):
    """(size_mb, max M sample-grads/s over W) — skips CRASH/missing cells."""
    out = []
...
def overlay_fig(systems):
    ...
    ax[0].set_xlabel('gradient workers W', fontsize=LABEL_SIZE)
    ax[0].set_ylabel('M sample-grads/s', fontsize=YLABEL_SIZE)
    ...
    ax[1].set_ylabel('M sample-grads/s', fontsize=YLABEL_SIZE)
    ax[1].margins(y=0.18)
    ...
    out = os.path.join(FIGS, 'ml_training_overlay.pdf')
    fig.savefig(out)
    print('wrote', out)
...
def bars_fig(systems):
    ...
    training latency (s). Per-panel auto-scaling keeps each size readable."""
    ...
    out = os.path.join(FIGS, 'ml_training_bars.pdf')
    fig.savefig(out)
    print('wrote', out)
